WITH
wt_tab70_1 AS (
  SELECT 'Zn'::varchar(5) AS metall, 2.49::float8 AS von, 2.71::float8 AS bis, 0.0::float8 AS wert1 UNION ALL
  SELECT 'Zn', 2.79, 3.21, 1.0 UNION ALL
  SELECT 'Zn', 3.29, 3.71, 1.5 UNION ALL
  SELECT 'Zn', 3.79, 4.21, 2.0 UNION ALL
  SELECT 'Zn', 4.29, 4.71, 2.5 UNION ALL
  SELECT 'Zn', 4.79, 5.21, 3.0 UNION ALL
  SELECT 'Zn', 5.29, 5.71, 4.0 UNION ALL
  SELECT 'Zn', 5.79, 6.21, 4.5 UNION ALL
  SELECT 'Zn', 6.29, 6.71, 5.0 UNION ALL
  SELECT 'Zn', 6.79, 8.01, 5.0
),

wt_tab70_2 AS (
  SELECT 'Zn'::varchar(5) AS metall, 5.5::float8 AS grenz_ph, 2.0::float8 AS humus, 3.0::float8 AS ton
),
...
UPDATE 
    ...
    SET 
    bindungsstaerke_zink = s.bindst_Zn_neu
FROM 
    select_table s
WHERE 
