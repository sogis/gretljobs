WITH
wt_tab70_1 AS (
  SELECT 'Cd'::varchar(5) AS metall, 2.49::float8 AS von, 2.71::float8 AS bis, 0.0::float8 AS wert1 UNION ALL
  SELECT 'Cd', 2.79, 3.21, 0.5 UNION ALL
  SELECT 'Cd', 3.29, 3.71, 1.0 UNION ALL
  SELECT 'Cd', 3.79, 4.21, 1.5 UNION ALL
  SELECT 'Cd', 4.29, 4.71, 2.0 UNION ALL
  SELECT 'Cd', 4.79, 5.21, 2.5 UNION ALL
  SELECT 'Cd', 5.29, 5.71, 3.5 UNION ALL
  SELECT 'Cd', 5.79, 6.21, 4.0 UNION ALL
  SELECT 'Cd', 6.29, 6.71, 4.5 UNION ALL
  SELECT 'Cd', 6.79, 8.01, 5.0 
),

wt_tab70_2 AS (
  SELECT 'Cd'::varchar(5) AS metall, 6.0::float8 AS grenz_ph, 4.0::float8 AS humus, 2.0::float8 AS ton
),
...
UPDATE 
    ...
    SET 
    bindungsstaerke_cadmium = s.bindst_cd_neu
FROM 
    select_table s
WHERE 
