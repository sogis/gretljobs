...
dokumente AS (
	...
	FROM
		...
		LEFT JOIN afu_schadstoffbelastete_boeden_v1.schdstfflstt_bden_stahlkonstruktion
...
SELECT
	...
	CASE
		WHEN schdstfflstt_bden_stahlkonstruktion.aktiv IS TRUE
			THEN 'Ja'
			ELSE 'Nein'
	END AS aktiv_txt,
	CASE
		WHEN schdstfflstt_bden_stahlkonstruktion.nutzungseinschraenkung IS TRUE
			THEN 'Ja'
			ELSE 'Nein'
	END AS nutzungseinschraenkung_txt,
	CASE
		WHEN schdstfflstt_bden_stahlkonstruktion.nutzungsverbot IS TRUE
			THEN 'Ja'
			ELSE 'Nein'
...
LEFT JOIN afu_schadstoffbelastete_boeden_v1.schdstfflstt_bden_stahlkonstruktionstyp konstruktionstyp
...
LEFT JOIN afu_schadstoffbelastete_boeden_v1.schadstoffbelasteter_boden_status status
	ON status.ilicode = schdstfflstt_bden_stahlkonstruktion.astatus
LEFT JOIN afu_schadstoffbelastete_boeden_v1.schadstoffbelasteter_boden_begruendung_aus_vsb_entlassen begruendung_vsb_entlassen
	ON begruendung_vsb_entlassen.ilicode = schdstfflstt_bden_stahlkonstruktion.begruendung_aus_vsb_entlassen
;
