...
FROM 
    ...
    WHERE 
        ...
        code.codeart_id = 19 
        AND 
        code.text::text = 'Landschaftsformen'::text) AS flaechenobjekte
    LEFT JOIN sogis_bln
        ON 
            ST_DWithin(sogis_bln.wkb_geometry, flaechenobjekte.geometrie, 0)
            AND
            sogis_bln.archive = 0
    LEFT JOIN richtplan.arp_basdat
        ON 
            ST_DWithin(arp_basdat.wkb_geometry, flaechenobjekte.geometrie, 0)
            AND
    ...
    LEFT JOIN richtplan.arp_natres
        ON 
            ST_DWithin(arp_natres.wkb_geometry, flaechenobjekte.geometrie, 0)
            AND
