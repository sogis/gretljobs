-- Unseren Datenbestand mit den heruntergeladenen Daten von infofauna aktualisieren mittels UPSERT (INSERT, bei Konflikt UPDATE).
-- Technischer Hinweis:
-- Wenn man *immer alle* Zielfelder überschreiben möchte, reicht INSERT OR REPLACE INTO ohne ON CONFLICT.
-- Aber wir wollen nur Zeilen einfügen bzw. aktualisieren, in denen sich tatsächlich etwas geändert hat (-> Logging!).
...
DO UPDATE SET
	-- Alle Felder ausser Konfliktfeld bzw. Primary Key müssen explizit gesetzt werden
