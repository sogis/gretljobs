...
FROM
    agi_lro_auflage.fixpunktekatgrie1_lfp1 AS lagefixpunkt
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie1_lfp1pos AS pos 
        ON pos.lfp1pos_von = lagefixpunkt.t_id
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie1_lfp1nachfuehrung AS nachfuehrung
...
SELECT
    ...
    trim(to_char(ST_X(lagefixpunkt.geometrie), '9999999.000'))||' / '||trim(to_char(ST_Y(lagefixpunkt.geometrie), '9999999.000')) AS koordinate
FROM
    agi_lro_auflage.fixpunktekatgrie2_lfp2 AS lagefixpunkt
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie2_lfp2pos AS pos 
        ON pos.lfp2pos_von = lagefixpunkt.t_id
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie2_lfp2nachfuehrung AS nachfuehrung
        ON lagefixpunkt.entstehung = nachfuehrung.t_id
    LEFT JOIN agi_lro_auflage.t_ili2db_basket AS basket
        ON lagefixpunkt.t_basket = basket.t_id
    LEFT JOIN aimport
...
SELECT
    ...
    trim(to_char(ST_X(lagefixpunkt.geometrie), '9999999.000'))||' / '||trim(to_char(ST_Y(lagefixpunkt.geometrie), '9999999.000')) AS koordinate
FROM
    agi_lro_auflage.fixpunktekatgrie3_lfp3 AS lagefixpunkt
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie3_lfp3pos AS pos 
        ON pos.lfp3pos_von = lagefixpunkt.t_id
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie3_lfp3nachfuehrung AS nachfuehrung
        ON lagefixpunkt.entstehung = nachfuehrung.t_id
    LEFT JOIN agi_lro_auflage.t_ili2db_basket AS basket
        ON lagefixpunkt.t_basket = basket.t_id
    LEFT JOIN aimport
...
FROM
    agi_lro_auflage.fixpunktekatgrie1_hfp1 AS hoehenfixpunkt
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie1_hfp1pos AS pos 
        ON pos.hfp1pos_von = hoehenfixpunkt.t_id
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie1_hfp1nachfuehrung AS nachfuehrung
...
SELECT
    ...
    trim(to_char(ST_X(hoehenfixpunkt.geometrie), '9999999.000'))||' / '||trim(to_char(ST_Y(hoehenfixpunkt.geometrie), '9999999.000')) AS koordinate
FROM
    agi_lro_auflage.fixpunktekatgrie2_hfp2 AS hoehenfixpunkt
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie2_hfp2pos AS pos 
        ON pos.hfp2pos_von = hoehenfixpunkt.t_id
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie2_hfp2nachfuehrung AS nachfuehrung
        ON hoehenfixpunkt.entstehung = nachfuehrung.t_id
    LEFT JOIN agi_lro_auflage.t_ili2db_basket AS basket
        ON hoehenfixpunkt.t_basket = basket.t_id
    LEFT JOIN aimport
...
SELECT
    ...
    trim(to_char(ST_X(hoehenfixpunkt.geometrie), '9999999.000'))||' / '||trim(to_char(ST_Y(hoehenfixpunkt.geometrie), '9999999.000')) AS koordinate
FROM
    agi_lro_auflage.fixpunktekatgrie3_hfp3 AS hoehenfixpunkt
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie3_hfp3pos AS pos 
        ON pos.hfp3pos_von = hoehenfixpunkt.t_id
    LEFT JOIN agi_lro_auflage.fixpunktekatgrie3_hfp3nachfuehrung AS nachfuehrung
        ON hoehenfixpunkt.entstehung = nachfuehrung.t_id
    LEFT JOIN agi_lro_auflage.t_ili2db_basket AS basket
        ON hoehenfixpunkt.t_basket = basket.t_id
    LEFT JOIN aimport
