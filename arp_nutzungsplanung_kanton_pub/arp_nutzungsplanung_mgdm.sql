...
--Grundnutzung wurde rausgenommen, da diese bei Kanton immer leer ist.
...
FROM 
    (
        ...
        FROM 
            arp_nutzungsplanung_kanton_v1.nutzungsplanung_ueberlagernd_flaeche AS flaeche
            JOIN arp_nutzungsplanung_mgdm_v1.geobasisdaten_typ AS typ 
            ON flaeche.typ_ueberlagernd_flaeche = typ.t_id 
        --WHERE 
        --    flaeche.t_datasetname <> '2403' FRAGE: WARUM??? 
    ) AS flaeche
WHERE 
    ST_Area(geometrie) > 0
;
...
FROM 
    (
        ...
        FROM 
            arp_nutzungsplanung_kanton_v1.nutzungsplanung_ueberlagernd_linie  AS linie
            JOIN arp_nutzungsplanung_mgdm_v1.geobasisdaten_typ AS typ 
            ON linie.typ_ueberlagernd_linie = typ.t_id             
        --WHERE 
        --    linie.t_datasetname <> '2403' FRAGE: WIESO??? 
...
FROM 
    (
        ...
        FROM 
            arp_nutzungsplanung_kanton_v1.erschlssngsplnung_erschliessung_linienobjekt  AS linie
            JOIN arp_nutzungsplanung_mgdm_v1.geobasisdaten_typ AS typ 
...
SELECT 
    ...
    typ.t_id 
FROM 
    arp_nutzungsplanung_kanton_v1.nutzungsplanung_ueberlagernd_punkt AS punkt
    JOIN arp_nutzungsplanung_mgdm_v1.geobasisdaten_typ AS typ 
    ON punkt.typ_ueberlagernd_punkt = typ.t_id 
--WHERE 
--    punkt.t_datasetname <> '2403' FRAGE: Wieso??? 
;
