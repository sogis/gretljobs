...
DELETE
  FROM ${BZ_SchemaName}.bauzonenstatistik_bebauungsstand_pro_gemeinde;

-- Aggregation der Kombination Gemeinde und Zonentyp, ohne Berechnung der Prozentwerte (wird in separatem Schritt berechnet)
INSERT INTO
  ${BZ_SchemaName}.bauzonenstatistik_bebauungsstand_pro_gemeinde
...
SELECT
  ...
  COALESCE((SELECT SUM(flaeche) FROM ${BZ_SchemaName}.bauzonenstatistik_bebauungsstand_mit_zonen_ohne_lsgrenzen WHERE bfs_nr = ol.bfs_nr AND bebauungsstand = 'bebaut' AND grundnutzung_typ_kt = ol.grundnutzung_typ_kt),0) AS bebaut_mit_zonen_ohne_lsgrenzen,
  COALESCE((SELECT SUM(flaeche) FROM ${BZ_SchemaName}.bauzonenstatistik_bebauungsstand_mit_zonen_ohne_lsgrenzen WHERE bfs_nr = ol.bfs_nr AND bebauungsstand = 'unbebaut' AND grundnutzung_typ_kt = ol.grundnutzung_typ_kt),0) AS unbebaut_mit_zonen_ohne_lsgrenzen,
  COALESCE((SELECT SUM(flaeche) FROM ${BZ_SchemaName}.bauzonenstatistik_bebauungsstand_mit_zonen_und_lsgrenzen WHERE bfs_nr = ol.bfs_nr AND bebauungsstand = 'bebaut' AND grundnutzung_typ_kt = ol.grundnutzung_typ_kt),0) AS bebaut_mit_zonen_und_lsgrenzen,
  COALESCE((SELECT SUM(flaeche) FROM ${BZ_SchemaName}.bauzonenstatistik_bebauungsstand_mit_zonen_und_lsgrenzen WHERE bfs_nr = ol.bfs_nr AND bebauungsstand = 'unbebaut' AND grundnutzung_typ_kt = ol.grundnutzung_typ_kt),0) AS unbebaut_mit_zonen_und_lsgrenzen,
  COALESCE((SELECT SUM(flaeche_beschnitten) FROM ${BZ_SchemaName}.bauzonenstatistik_liegenschaft_nach_bebauungsstand WHERE bfs_nr = ol.bfs_nr AND bebauungsstand IN ('bebaut','teilweise_bebaut') AND grundnutzung_typ_kt = ol.grundnutzung_typ_kt),0) AS bebaut_aus_liegenschaftszuteilung,
  COALESCE((SELECT SUM(flaeche_beschnitten) FROM ${BZ_SchemaName}.bauzonenstatistik_liegenschaft_nach_bebauungsstand WHERE bfs_nr = ol.bfs_nr AND bebauungsstand = 'unbebaut' AND grundnutzung_typ_kt = ol.grundnutzung_typ_kt),0) AS unbebaut_aus_liegenschaftszuteilung,
...
FROM ${BZ_SchemaName}.bauzonenstatistik_bebauungsstand_mit_zonen_ohne_lsgrenzen ol
