...
-- *********************************************************************************
    ...
    WITH getStichtage AS (
        SELECT
            BTRIM(lower(to_char((stichtag), 'Day'))) AS stichtag
        FROM
    ...
    calendar AS (
        ...
        SELECT
            service_id,
        CASE
            WHEN stichtag = 'tuesday'
                THEN gtfs_calendar.tuesday
            WHEN stichtag = 'thursday'
                THEN gtfs_calendar.thursday
        END AS weekofday99
        FROM
