...
INSERT
    INTO export.strukturdaten_zonenschild (
        geometrie,
        flaeche,
        flaeche_bebaut,
        flaeche_unbebaut,
    ...
    SELECT
        ...
        b.bodenbedeckungen
    FROM
        export.zonenschild_geoms a
    LEFT JOIN export.zonenschild_bodenbedeckungen_json b
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_gwr_json c
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_gebaeude_statistik d
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_wohnungen_statistik e
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_grundnutzungen_json f
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_statpop_json h
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_statent_statistik i
        USING (schild_uuid)
