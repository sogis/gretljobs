SET search_path to agi_adressen_pub_v2, public;
...
index_base AS (
    SELECT
        ...
        id AS id_in_class,
        concat(strassenname, ' | ', plz, ' ' , ortschaft, ' (Strasse)' )  AS displaytext,
        concat(strassenname, ' | ', plz, ' ' , ortschaft, ' (Strasse)' ) AS part_1,
        'Strassenachse Name '::text AS part_3,
        (st_asgeojson(st_envelope(geometrie), 0, 1)::json -> 'bbox'::text)::text AS bbox
    FROM
        agi_adressen_pub_v2.adressen_strassenachse
    WHERE
