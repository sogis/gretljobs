...
Sondern das Pod Template dieses Agents wird zwar referenziert (`inheritFrom env.NODE_LABEL ?: 'gretl'` auf Zeile 4), aber mit zusätzlichem YAML wird ein weiterer Container mit Name `processing-db` und Image `postgis/postgis:${env.POSTGIS_IMAGE_TAG}` definiert, der zusätzlich zum GRETL-Container im gleichen Pod gestartet wird.
