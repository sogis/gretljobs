SELECT a.ogc_fid AS ogc_fid, a.xkoord AS xkoord, a.ykoord AS ykoord, a.wkb_geometry, a.gem_bfs AS gem_bfs, 
    sum(a.emiss_co) AS emiss_co, sum(a.emiss_co2) AS emiss_co2, 
    sum(a.emiss_nox) AS emiss_nox, sum(a.emiss_so2) AS emiss_so2, 
    sum(a.emiss_nmvoc) AS emiss_nmvoc, sum(a.emiss_ch4) AS emiss_ch4, 
    sum(a.emiss_pm10) AS emiss_pm10, sum(a.emiss_nh3) AS emiss_nh3, 
    sum(a.emiss_n2o) AS emiss_n2o, sum(a.emiss_xkw) AS emiss_xkw 
FROM (
   ...
   FROM ( SELECT a.ogc_fid, a.xkoord, a.ykoord, a.wkb_geometry, a.gem_bfs, 
            b.corinaircd, 
                ...
                CASE
                    ...
                    ELSE 0::numeric
                END AS emiss_xkw
           FROM ekat2015.ha_raster_100 a -- Alle Kacheln
      LEFT JOIN ( SELECT a.xkoord, a.ykoord, a.emiss, a.schast, a.schast_nr, 
                  ...
                  WHERE a.corinaircd::text <> 'Metallreinigung'::text AND a.corinaircd::text <> 'Lachgasanwendung Spitäler'::text AND a.corinaircd::text <> 'Farbanwendung Bau'::text AND a.anldatid <> 324 AND NOT (emibetid = 1813 AND anldatid = 322)) b ON a.xkoord = b.xkoord AND a.ykoord = b.ykoord -- Ohne stationäre Baumaschinen (322) von Stahlwerk Gerlafingen (1813)!
     WHERE a.archive = 0) e
...
GROUP BY a.ogc_fid, a.xkoord, a.ykoord, a.wkb_geometry, a.gem_bfs
