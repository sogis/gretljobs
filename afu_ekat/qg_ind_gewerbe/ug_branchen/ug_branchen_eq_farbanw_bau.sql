 SELECT a.ogc_fid, a.xkoord, a.ykoord, a.wkb_geometry, a.gem_bfs, 
        CASE
         ...
         WHERE d.equelle_id = 113 AND e.s_code = 22 AND d.archive = 0 AND e.archive = 0)) / c.ma_ch::double precision * 1000::double precision * f.anz_arbpl::numeric::double precision
   ...
   LEFT JOIN ( SELECT sum(bz_noga_art_ha.anz_arbpl) AS anz_arbpl, 
          ...
          WHERE substr(bz_noga_art_ha.noga_art, 1, 4) = '4120'::text
    ...
    ( SELECT sum(bz_noga_art_summe.ma_ch) AS ma_ch
     ...
     WHERE substr(bz_noga_art_summe.noga_art, 1, 4) = '4120'::text) c
  WHERE a.archive = 0
