 SELECT c.ogc_fid, c.xkoord, c.ykoord, c.wkb_geometry, c.gem_bfs, 
        CASE
            WHEN d.art = 26 THEN (( SELECT quelle_emiss_efak.schast_efak
               FROM ekat2015.quelle_emiss_efak
              WHERE quelle_emiss_efak.equelle_id = 72 AND quelle_emiss_efak.s_code = 2 AND quelle_emiss_efak.archive = 0)) * d.flaeche
            ELSE 0::numeric
        END AS emiss_nox, 
        CASE
            WHEN d.art = 26 THEN (( SELECT quelle_emiss_efak.schast_efak
               FROM ekat2015.quelle_emiss_efak
              WHERE quelle_emiss_efak.equelle_id = 72 AND quelle_emiss_efak.s_code = 6 AND quelle_emiss_efak.archive = 0)) * d.flaeche
            ELSE 0::numeric
        END AS emiss_ch4, 
        CASE
            WHEN d.art = 26 THEN (( SELECT quelle_emiss_efak.schast_efak
               FROM ekat2015.quelle_emiss_efak
              WHERE quelle_emiss_efak.equelle_id = 72 AND quelle_emiss_efak.s_code = 26 AND quelle_emiss_efak.archive = 0)) * d.flaeche
            ELSE 0::numeric
        END AS emiss_n2o
   ...
   LEFT JOIN ( SELECT min(intersec_bdbed_ha_raster.art) AS art, 
          ...
          WHERE intersec_bdbed_ha_raster.archive = 0 AND intersec_bdbed_ha_raster.art = 26
