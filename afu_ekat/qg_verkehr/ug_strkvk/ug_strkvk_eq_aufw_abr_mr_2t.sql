SELECT min(k.ogc_fid) AS ogc_fid, min(k.xkoord) AS xkoord, 
   ...
   FROM ( SELECT a.ogc_fid, a.xkoord, a.ykoord, a.wkb_geometry, a.gem_bfs, 
                CASE
                    WHEN c.ioaoab::text = 'AO'::text THEN 0.38::double precision * b.dtv2015_mr::double precision * (( SELECT b.schast_efak
                 ...
                 WHERE a.equelle_id = 49 AND b.s_code = 7 ))::double precision * 365::double precision
                    WHEN c.ioaoab::text = 'IO'::text THEN 0.38::double precision * b.dtv2015_mr::double precision * (( SELECT b.schast_efak
                       FROM ekat2015.emiss_quelle a
                  LEFT JOIN ekat2015.quelle_emiss_efak b ON a.equelle_id = b.equelle_id
