SELECT min(c.ogc_fid) AS ogc_fid, min(c.xkoord)::numeric AS xkoord, 
   ...
   FROM ( SELECT k.ogc_fid, k.xkoord, k.ykoord, k.wkb_geometry, k.gem_bfs, 
                CASE
                 ...
                 WHERE a.equelle_id = 62 AND b.s_code = 7 AND a.archive = 0 AND b.archive = 0)) * (k.anz_mr_4t / (k.anz_smw + k.anz_lmw + k.anz_mr_2t + k.anz_mr_4t)))::double precision * (k.dtv_pw + k.dtv_li) * k.laenge * 365::numeric::double precision) / (k.arb_plaetze_zone + k.einw_zone)::double precision
