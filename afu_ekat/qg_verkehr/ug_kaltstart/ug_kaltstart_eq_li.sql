   ...
   FROM ( SELECT k.ogc_fid, k.xkoord, k.ykoord, k.wkb_geometry, k.gem_bfs, 
                CASE
                    WHEN k.arb_plaetze_zone > 0::numeric OR k.einw_zone > 0::numeric THEN (k.p00btot + k.b05empts2 + k.b05empts3)::double precision * ((( SELECT efak_kalt_tank.efak
                      ...
                      WHERE efak_kalt_tank.efak_code = 1 AND efak_kalt_tank.schast = 1 AND efak_kalt_tank.fhz_art = 'LNF'::text AND efak_kalt_tank.archive = 0)) * k.starts_li * 365::double precision / 1000::double precision) / (k.arb_plaetze_zone + k.einw_zone)::double precision
                    ELSE 0::double precision
                END AS emiss_so2, 
                CASE
                    WHEN k.arb_plaetze_zone > 0::numeric OR k.einw_zone > 0::numeric THEN (k.p00btot + k.b05empts2 + k.b05empts3)::double precision * ((( SELECT efak_kalt_tank.efak
                       FROM ekat2015.efak_kalt_tank
                      WHERE efak_kalt_tank.efak_code = 1 AND efak_kalt_tank.schast = 2 AND efak_kalt_tank.fhz_art = 'LNF'::text AND efak_kalt_tank.archive = 0)) * k.starts_li * 365::double precision / 1000::double precision) / (k.arb_plaetze_zone + k.einw_zone)::double precision
                    ELSE 0::double precision
                END AS emiss_nox, 
                CASE
                    WHEN k.arb_plaetze_zone > 0::numeric OR k.einw_zone > 0::numeric THEN (k.p00btot + k.b05empts2 + k.b05empts3)::double precision * ((( SELECT efak_kalt_tank.efak
                       FROM ekat2015.efak_kalt_tank
                      WHERE efak_kalt_tank.efak_code = 1 AND efak_kalt_tank.schast = 5 AND efak_kalt_tank.fhz_art = 'LNF'::text AND efak_kalt_tank.archive = 0)) * k.starts_li * 365::double precision / 1000::double precision) / (k.arb_plaetze_zone + k.einw_zone)::double precision
                    ELSE 0::double precision
                END AS emiss_co, 
                CASE
                    WHEN k.arb_plaetze_zone > 0::numeric OR k.einw_zone > 0::numeric THEN (k.p00btot + k.b05empts2 + k.b05empts3)::double precision * ((( SELECT efak_kalt_tank.efak
                       FROM ekat2015.efak_kalt_tank
                      WHERE efak_kalt_tank.efak_code = 1 AND efak_kalt_tank.schast = 6 AND efak_kalt_tank.fhz_art = 'LNF'::text AND efak_kalt_tank.archive = 0)) * k.starts_li * 365::double precision / 1000::double precision) / (k.arb_plaetze_zone + k.einw_zone)::double precision
                    ELSE 0::double precision
                END AS emiss_ch4, 
                CASE
                    WHEN k.arb_plaetze_zone > 0::numeric OR k.einw_zone > 0::numeric THEN (k.p00btot + k.b05empts2 + k.b05empts3)::double precision * ((( SELECT efak_kalt_tank.efak
                       FROM ekat2015.efak_kalt_tank
                      WHERE efak_kalt_tank.efak_code = 1 AND efak_kalt_tank.schast = 7 AND efak_kalt_tank.fhz_art = 'LNF'::text AND efak_kalt_tank.archive = 0)) * k.starts_li * 365::double precision / 1000::double precision) / (k.arb_plaetze_zone + k.einw_zone)::double precision
                    ELSE 0::double precision
                END AS emiss_pm10, 
                CASE
                    WHEN k.arb_plaetze_zone > 0::numeric OR k.einw_zone > 0::numeric THEN (k.p00btot + k.b05empts2 + k.b05empts3)::double precision * ((( SELECT efak_kalt_tank.efak
                       FROM ekat2015.efak_kalt_tank
                      WHERE efak_kalt_tank.efak_code = 1 AND efak_kalt_tank.schast = 22 AND efak_kalt_tank.fhz_art = 'LNF'::text AND efak_kalt_tank.archive = 0)) * k.starts_li * 365::double precision / 1000::double precision) / (k.arb_plaetze_zone + k.einw_zone)::double precision
                    ELSE 0::double precision
                END AS emiss_nmvoc
