SELECT min(k.ogc_fid) AS min, min(k.xkoord) AS xkoord, min(k.ykoord) AS ykoord, 
   ...
   FROM ( SELECT a.ogc_fid, a.xkoord, a.ykoord, a.wkb_geometry, a.gem_bfs, 
                CASE
                    WHEN a.xkoord = b.xkoord::numeric AND a.ykoord = b.ykoord::numeric THEN b.dtv2015_li * (0.3333333 * ((( SELECT efak_kalt_tank.efak
                       FROM ekat2015.efak_kalt_tank
                      WHERE efak_kalt_tank.efak_code = 2 AND efak_kalt_tank.fhz_art = 'LNF'::text AND efak_kalt_tank.archive = 0))::numeric * d.anz_lnf_li::numeric * 365::numeric) / 1000::numeric / c.dtv_li_gem)
                    ELSE 0::numeric
                END AS emiss_nmvoc
