SELECT min(c.ogc_fid) AS min, min(c.xkoord)::numeric AS xkoord, 
   ...
   FROM ( SELECT k.ogc_fid, k.xkoord, k.ykoord, k.wkb_geometry, k.gem_bfs, 
                CASE
                    WHEN k.arb_plaetze_zone > 0::numeric OR k.einw_zone > 0::numeric THEN (k.p00btot + k.b05empts2 + k.b05empts3)::double precision * ((( SELECT efak_kalt_tank.efak
                      ...
                      WHERE efak_kalt_tank.efak_code = 3 AND efak_kalt_tank.fhz_art = 'PKW'::text AND efak_kalt_tank.archive = 0)) * k.stops_pw * 365::double precision / 1000::double precision) / (k.arb_plaetze_zone + k.einw_zone)::double precision
                    ELSE 0::double precision
                END AS emiss_nmvoc
           FROM ( SELECT a.ogc_fid, a.xkoord, a.ykoord, 
                    a.wkb_geometry_ha_rast AS wkb_geometry, a.gem_bfs, 
                    b.arb_plaetze_zone, b.einw_zone, b.stops_pw, a.vk_zone, 
