SELECT k.ogc_fid, k.xkoord, k.ykoord, k.wkb_geometry, k.gem_bfs, 
        CASE
            WHEN k.besch_gesamt > 0::double precision OR k.b15btot > 0 THEN (k.b15btot::double precision + k.besch_gesamt)::numeric * ((0.6666666 * ((( SELECT efak_kalt_tank.efak
              ...
              WHERE efak_kalt_tank.efak_code = 2 AND efak_kalt_tank.fhz_art = 'KR/MR'::text AND efak_kalt_tank.archive = 0))::numeric * k.anz_mr::numeric * 365::numeric) / 1000::numeric)::double precision / (k.besch_gesamt_gem::double precision + k.einw_gem::double precision))::numeric
            ELSE 0::numeric
        END AS emiss_nmvoc
   FROM ( SELECT a.ogc_fid, a.xkoord, a.ykoord, a.wkb_geometry, a.gem_bfs, 
            e.besch_gesamt_gem, h.einw_gem, f.anz_mr, 
                CASE
   ...
   LEFT JOIN ( SELECT gmde.ogc_fid, gmde.gem_bfs, gmde.anz_mr
   FROM ekat2015.gmde
