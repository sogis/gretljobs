...
INSERT
    ...
    SELECT
        ...
        b.bodenbedeckungen
    FROM
        export.zonenschild_basis a
    LEFT JOIN export.zonenschild_bodenbedeckungen_array b
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_gwr_array c
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_gwr_geb_agg d
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_gwr_wohn_agg e
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_grundnutzungen_array f
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_statpop_array h
        USING (schild_uuid)
    LEFT JOIN export.zonenschild_statent_agg i
        USING (schild_uuid)
