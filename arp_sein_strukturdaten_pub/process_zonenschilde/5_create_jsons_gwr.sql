...
DROP TABLE IF EXISTS
    export.zonenschild_gwr_json CASCADE
;

CREATE TABLE
    export.zonenschild_gwr_json AS
    ...
    FROM
        export.zonenschild_geoms p
        LEFT JOIN gkat_json USING (schild_uuid)
        LEFT JOIN gklas_10_json USING (schild_uuid)
        LEFT JOIN gbaup_json USING (schild_uuid)
        LEFT JOIN wazim_json USING (schild_uuid)
;

CREATE INDEX
    ON export.zonenschild_gwr_json (schild_uuid)
;