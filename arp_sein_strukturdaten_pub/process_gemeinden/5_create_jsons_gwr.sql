...
DROP TABLE IF EXISTS
    export.gemeinde_gwr_json CASCADE
;

CREATE TABLE
    export.gemeinde_gwr_json AS
    ...
    FROM
        export.gemeinde_geoms p
        LEFT JOIN gkat_json USING (bfs_nr)
        LEFT JOIN gklas_10_json USING (bfs_nr)
        LEFT JOIN gbaup_json USING (bfs_nr)
        LEFT JOIN wazim_json USING (bfs_nr)
;

CREATE INDEX
    ON export.gemeinde_gwr_json (bfs_nr)
;