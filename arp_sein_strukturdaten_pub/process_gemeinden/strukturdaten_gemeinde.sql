...
INSERT
    ...
    SELECT
        ...
        b.bodenbedeckungen
    FROM
        export.gemeinde_basis a
    LEFT JOIN export.gemeinde_bodenbedeckungen_array b
        USING (bfs_nr)
    LEFT JOIN export.gemeinde_gwr_array c
        USING (bfs_nr)
    LEFT JOIN export.gemeinde_gwr_geb_agg d
        USING (bfs_nr)
    LEFT JOIN export.gemeinde_gwr_wohn_agg e
        USING (bfs_nr)
    LEFT JOIN export.gemeinde_grundnutzungen_array f
        USING (bfs_nr)
    LEFT JOIN export.gemeinde_statpop_array h
        USING (bfs_nr)
    LEFT JOIN export.gemeinde_statent_agg i
        USING (bfs_nr)
