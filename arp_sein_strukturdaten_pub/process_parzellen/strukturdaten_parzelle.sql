...
INSERT
    ...
    SELECT
        ...
        b.bodenbedeckungen
    FROM
        export.parzellen_basis a
    LEFT JOIN export.parzellen_bodenbedeckungen_array b
        USING (t_ili_tid)
    LEFT JOIN export.parzellen_gwr_array c
        USING (t_ili_tid)
    LEFT JOIN export.parzellen_gwr_geb_agg d
        USING (t_ili_tid)
    LEFT JOIN export.parzellen_gwr_wohn_agg e
        USING (t_ili_tid)
    LEFT JOIN export.parzellen_grundnutzungen_array f
        USING (t_ili_tid)
    LEFT JOIN export.parzellen_statpop_array h
        USING (t_ili_tid)
    LEFT JOIN export.parzellen_statent_agg i
        USING (t_ili_tid)
