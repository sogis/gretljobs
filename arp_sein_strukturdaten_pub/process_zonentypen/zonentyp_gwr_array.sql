-- GWR GEBÄUDE: Aggregiert auf Zonentyp-Ebene (Arrays)

DROP TABLE IF EXISTS
    export.zonentyp_gwr_array CASCADE
;

CREATE TABLE
    export.zonentyp_gwr_array AS
    ...
    FROM
        export.zonentyp_basis p
        LEFT JOIN gkat_array USING (typ_kt, bfs_nr)
        LEFT JOIN gklas_10_array USING (typ_kt, bfs_nr)
        LEFT JOIN gbaup_array USING (typ_kt, bfs_nr)
        LEFT JOIN wazim_array USING (typ_kt, bfs_nr)
;

CREATE INDEX
    ON export.zonentyp_gwr_array (typ_kt, bfs_nr)
;